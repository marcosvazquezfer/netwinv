...
class PDFWriter:
    ...
    The exported PDF can be written in either english or spanish.
    ...
    def __build_file_name(self):
        ...
        if not self.__output_directory.endswith('/'):
            return 'data/' + self.__output_directory + '/' + title + '.pdf'
        else:
            return 'data/' + self.__output_directory[:-1] + '/' + title + '.pdf'
    ...
    def write_document(self):
        """ Writes the entire PDF dcoument, from the php.ini and the information previously loaded.
        """
